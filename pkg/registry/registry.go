...
import (
	...
	"github.com/dhnt/oci/pkg/registry/net"
	"github.com/dhnt/oci/pkg/registry/store"
)
...
func (r *registry) resolve(repo, reference string) ([]string, error) {
	...
	if cid := ToB32(repo); cid != "" {
		return []string{cid}, nil
	}
	if hash := IpfsifyHash(repo); hash != "" {
		if cid := ToB32(hash); cid != "" {
			return []string{cid}, nil
		}
	}
...
func New(config *Config, opts ...Option) http.Handler {
	imageStore := store.Create(config.Store)
